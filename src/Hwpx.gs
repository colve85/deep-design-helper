 ...
 */
function hwpxFromBlocks(blocks) {
...
function hwpxTable_(b, ctx) {
  ...
  var used = 0;
  for (i = 0; i < colCnt; i++) {
    var w = weights ? Math.floor(HWPX_TEXT_WIDTH * weights[i] / total)
                    : Math.floor(HWPX_TEXT_WIDTH / colCnt);
    widths.push(w); used += w;
  }
  widths[colCnt - 1] += (HWPX_TEXT_WIDTH - used);   // 반올림 오차 보정
  ...
  var tbl = '<hp:tbl id="' + (ctx.id++) + '" zOrder="0" numberingType="TABLE"' +
    ...
    '<hp:sz width="' + HWPX_TEXT_WIDTH + '" widthRelTo="ABSOLUTE" height="' + totalH +
